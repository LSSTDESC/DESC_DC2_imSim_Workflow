...
def determine_instcat_work(instcat_list, outfile):
    ...
    ddf_region = trim_sensors.Run20Region(ra_mid=53.12, ne_corner =(56.62, -24.6), dec_range=(-31.6,-24.6))
    ...
    flags_ddf = [len(ddf_region.trim_sensors(instcat)) != 0 for instcat in instcat_list]
    work_data = [ [instcat, sensors] for (instcat, sensors) in zip(instcat_list, sensors_list)]
    for work, flag in zip(work_data, flags_ddf):
