...
def singularity_wrapper(img, inst_cat_root, work_and_out_path, cmd):
  wrapped_cmd = "singularity exec -B {},{},/projects/LSSTsky {} /projects/LSSTsky/Run3.0i/DESC_DC2_imSim_Workflow/docker_run.sh {}".format(inst_cat_root, work_and_out_path, img, cmd)
  return wrapped_cmd

@bash_app(executors=['submit-node'])
def validate_transfer(wrap, inst_cat_root: str, tarball_json: str):
    base = "/projects/LSSTsky/Run3.0i/DESC_DC2_imSim_Workflow/scripts/parsl-validate-transfer.py {} {}".format(inst_cat_root, tarball_json)
    c = wrap(base)
